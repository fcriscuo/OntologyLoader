...
def extract_pubmed_data(pubmed_article):
    ...
    if abstract != "" and abstract is not None:
        embedding = ges.generate_embedding(title, abstract)
        return {"pmid": pmid, "url": url, "title": title, "abstract": abstract, "journal": journal,
            ...
            "pmc_url": pmc_url, "doi": doi, "references": references, "needs_properties": False, "embedding": embedding}
...
def extract_author_names(pubmed_article):
    ...
    for author in authors:
        ...
        last_name = pmf.remove_single_quotes(last_name)
        initial = author.find("Initials").text if author.find(
            "Initials") is not None else ""
        initial = pmf.remove_single_quotes(initial)
...
def persist_pubmed_data(pubmed_data):
    ...
    id_prop = "id"
    id_value = f"PMID:{pubmed_data['pmid']}"
    print(f"Updating node {label} {id_prop} {id_value} {pubmed_data['title']}")
    pmf.update_node(label, id_prop, id_value, pubmed_data)

# Define a main method for testing
def main():
    pubmed_ids = [11471732,19376779, 8948441,16115814]
    ...
    for pubmed_article in pubmed_articles:
        ...
        #print(f"{data}\n")
        print(f"Processing PMID {data['pmid']}  {data['title']}  {data['abstract']}")
